...
class hBT {
	...
	public BoardGUI cb;
	public ArrayList<HitoriPuzzle[][]> results;
	...
	public ArrayList<HitoriPuzzle> createDomain(int size){
		...
		for(int row=0;row<size;row++){
			for(int col=0;col<size;col++){
				if(repeated(row, col, board[row][col].getValue(), size) && !board[row][col].isShaded()){
	...
	public boolean solve(int size, ArrayList<HitoriPuzzle> domain) throws InterruptedException{
			...
			for(HitoriPuzzle d:domain){
				...
				if(!neighboursNotShaded(row, col, size) || isIsland(row, col, size)){
				}
				else{
					d.setShaded(true);
	...
	private void uncheckAll(int size) {
		for(int i=0;i<size;i++){
			for(int j=0;j<size;j++){
	...
	private boolean isIsland(int row, int col, int size) {
		...
		for (int i = 0; i < neighbors.length; i++) {
			...
			if (neighbors[i]==null || neighbors[i].isChecked()
